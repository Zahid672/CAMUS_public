# train_attention_unet_cbam_se_ce_dice_focal.py
import os
import csv
import time
...
RESULTS_DIR = "AttUNet_CBAM_SE_results_CE_DICE_FOCAL"
METRICS_CSV = os.path.join(RESULTS_DIR, "metrics_ce_dice_focal.csv")
QUAL_DIR    = "qualitative_AttUNet_CBAM_SE_CE_DICE_FOCAL"

# -------- loss weights (feel free to tune) --------
CE_WEIGHT     = 0.4
DICE_WEIGHT   = 0.4
FOCAL_WEIGHT  = 0.2
FOCAL_GAMMA   = 2.0
FOCAL_ALPHA   = None   # None | scalar | per-class list/np.array/1D tensor
...
# -------------- Soft Dice Loss (multi-class, logits) --------------
class SoftDiceLoss(nn.Module):
    """
    Multi-class soft Dice computed on logits (via softmax).
    Returns 1 - mean Dice across classes.
    """
    ...
    def forward(self, logits: torch.Tensor, target_hw: torch.Tensor) -> torch.Tensor:
        ...
        if self.ignore_index is not None:
            valid = (target_hw != self.ignore_index).float().unsqueeze(1)  # [B,1,H,W]
            probs = probs * valid
            tgt   = tgt * valid
...
# -------------- Focal Loss (multi-class, logits) --------------
class FocalLossCE(nn.Module):
    """
    logits -> per-pixel CE (no reduction) -> focal weighting -> mean.
    alpha can be None | scalar | per-class 1D tensor/list/np.array.
    """
...
@torch.no_grad()
def evaluate(model, loader, criterion, device, num_classes, img_size,
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        te_loss, per_cls_dice, mDice, per_cls_iou, mIoU, cm = evaluate(
            model, val_loader, criterion, DEVICE, NUM_CLASSES, IMG_SIZE,
