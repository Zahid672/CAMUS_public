# train_attention_unet_cbam_se_focal.py
import os
import csv
import time
...
RESULTS_DIR = "AttUNet_CBAM_SE_results_FOCAL"
METRICS_CSV = os.path.join(RESULTS_DIR, "metrics_focal.csv")
QUAL_DIR    = "qualitative_AttUNet_CBAM_SE_FOCAL"
...
class FocalLossCE(nn.Module):
    """
    Standard multi-class focal loss applied on per-pixel cross-entropy.
    logits -> CE (no reduction) -> focal weight -> mean.
    alpha can be: None, scalar float, or a 1D tensor/list of per-class weights.
    """
...
# -------------- train / eval loops (Focal only) --------------
def train_one_epoch(model, loader, optimizer, focal_loss, device, img_size):
    model.train()
    running = 0.0
...
@torch.no_grad()
def evaluate(model, loader, focal_loss, device, num_classes, img_size,
...
def main():
    ...
    # -------- Loss: Focal ONLY --------
    # You can tune gamma (2.0 default). Optional alpha: scalar or per-class list/tensor.
    focal_loss = FocalLossCE(gamma=2.0, alpha=None, ignore_index=-100)
    ...
    for epoch in range(EPOCHS):
        tr_loss = train_one_epoch(model, train_loader, optimizer, focal_loss, DEVICE, IMG_SIZE)
        curr_lr = optimizer.param_groups[0]['lr']

        te_loss, per_cls_dice, mDice, per_cls_iou, mIoU, cm = evaluate(
            model, val_loader, focal_loss, DEVICE, NUM_CLASSES, IMG_SIZE,
            save_dir=QUAL_DIR, epoch=epoch+1, save_n=SAVE_N
        )

        print(
            f"[AttUNet-CBAM-SE + Focal] Epoch {epoch+1:03d} | "
        ...
        if mDice > best_mdice:
            ...
            torch.save(model.state_dict(), f"best_attunet_cbam_se_focal_mdice_{best_mdice:.4f}.pt")
        else:
            bad += 1
            if bad >= patience:
