...
def main_logits_from(output: Union[torch.Tensor, Tuple[torch.Tensor, ...]]) -> torch.Tensor:
    """Support models that return (main, aux1, aux2)."""
...
def train_one_epoch(model, loader, optimizer, criterion_ce, device, img_size):
    ...
    for imgs, masks in loader:
        ...
        # align logits to target HxW if necessary
...
def main():
    ...
    criterion_ce = nn.CrossEntropyLoss(weight=class_weights)  # targets: [B,H,W] long
    ...
    for epoch in range(EPOCHS):
        ...
        # checkpoint by mDice (even though loss is CE-only)
        if mDice > best_mdice:
            best_mdice = mDice; bad = 0
            torch.save(model.state_dict(), f"best_unet_ce_mdice_{best_mdice:.4f}.pt")
        else:
            bad += 1
            if bad >= patience:
