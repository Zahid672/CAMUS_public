# train_from_png_ce_dice_focal.py
import os
import csv
import time
import random
...
# ===================== CLI =====================
def get_args():
    ap = argparse.ArgumentParser("Train Attention U-Net (CBAM+SE) on CAMUS PNGs")
    ...
    ap.add_argument("--results", type=str, default="AttUNet_CBAM_SE_results_CE_DICE_FOCAL")
    ap.add_argument("--qual", type=str, default="qualitative_AttUNet_CBAM_SE_CE_DICE_FOCAL")
    ap.add_argument("--ce_w", type=float, default=0.4, help="weight for CE")
    ap.add_argument("--dice_w", type=float, default=0.4, help="weight for Dice")
    ap.add_argument("--focal_w", type=float, default=0.2, help="weight for Focal")
    ap.add_argument("--gamma_focal", type=float, default=2.0, help="focal gamma")
...
class FocalLossCE(nn.Module):
    def __init__(self, gamma: float = 2.0, alpha=None, ignore_index: int = -100):
        super().__init__()
    ...
    def forward(self, logits, target):
        ...
        if self.ignore_index is not None and self.ignore_index >= 0:
            ...
            if valid.sum() == 0:
                return ce.new_zeros(())
            ce = ce[valid]; target = target[valid]
        pt = torch.exp(-ce)
        focal = (1.0 - pt) ** self.gamma * ce
        if self.alpha is not None:
            if isinstance(self.alpha, (list, tuple, np.ndarray)):
                alpha_vec = torch.tensor(self.alpha, dtype=focal.dtype, device=logits.device)
                focal = alpha_vec[target] * focal
            elif torch.is_tensor(self.alpha):
                focal = self.alpha.to(logits.device, dtype=focal.dtype)[target] * focal
            else:
                focal = float(self.alpha) * focal
        return focal.mean()

class CEDiceFocalLoss(nn.Module):
    def __init__(self, ce_w=0.4, dice_w=0.4, focal_w=0.2, class_weights=None, ignore_index=-100, dice_smooth=1.0, focal_gamma=2.0):
        super().__init__()
        self.ce_w, self.dice_w, self.focal_w = float(ce_w), float(dice_w), float(focal_w)
        self.ce    = nn.CrossEntropyLoss(weight=class_weights, ignore_index=ignore_index)
        self.dice  = SoftDiceLoss(smooth=dice_smooth, ignore_index=ignore_index)
        self.focal = FocalLossCE(gamma=focal_gamma, alpha=None, ignore_index=ignore_index)
...
def split_indices_by_patient(ds: CAMUSPNGSliceDataset, train_frac=0.8, seed=42):
    ...
    # map patient_dir -> list of indices
    buckets = {}
    for idx, (img_path, mask_path) in enumerate(ds.items):
        # parent folder name (e.g., patient0001)
...
def main():
    ...
    criterion = CEDiceFocalLoss(
        ce_w=args.ce_w, dice_w=args.dice_w, focal_w=args.focal_w,
        class_weights=None, ignore_index=-100, dice_smooth=1.0, focal_gamma=args.gamma_focal
    )
    ...
    patience, bad = 20, 0
    t0 = time.time()
    metrics_csv = os.path.join(args.results, "metrics_ce_dice_focal.csv")
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"[AttUNet-CBAM-SE CE+Dice+Focal] Epoch {epoch+1:03d} | "
        ...
        if mDice > best_mdice:
            ...
            torch.save(model.state_dict(), f"best_attunet_cbam_se_ce_dice_focal_mdice_{best_mdice:.4f}.pt")
        else:
            bad += 1
            if bad >= patience:
