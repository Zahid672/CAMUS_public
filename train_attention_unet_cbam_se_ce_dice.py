# train_attention_unet_cbam_se_ce_dice.py
import os
import csv
import time
...
RESULTS_DIR = "AttUNet_CBAM_SE_results_CE_DICE"
METRICS_CSV = os.path.join(RESULTS_DIR, "metrics_ce_dice.csv")
QUAL_DIR    = "qualitative_AttUNet_CBAM_SE_CE_DICE"
...
class CEDiceLoss(nn.Module):
    def __init__(self, ce_weight=0.5, dice_weight=0.5, class_weights=None, ignore_index: int = -100, smooth=1.0):
        super().__init__()
        self.ce_weight   = float(ce_weight)
        self.dice_weight = float(dice_weight)
        self.ce = nn.CrossEntropyLoss(weight=class_weights, ignore_index=ignore_index)
        self.dice = SoftDiceLoss(smooth=smooth, ignore_index=ignore_index)
...
def main():
    ...
    criterion = CEDiceLoss(ce_weight=CE_WEIGHT,
